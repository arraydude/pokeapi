...
import { PokemonResult, IAPIPokemonList } from "types";
...
function useListPokemons(
  ...
  const [pokeData, setPokeData] = useState<IAPIPokemonList | undefined>();
  ...
  async function fetch(limit: number, offset: number) {
    ...
    try {
      const { data } = await axios.get<IAPIPokemonList>(
